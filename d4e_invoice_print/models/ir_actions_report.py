...
class Report(models.Model):
    ...
    def _render_qweb_pdf(self, res_ids=None, data=None):
        ...
        if report_type == 'qweb-pdf' and report_name in ['d4e_invoice_print.account_invoices_with_isr', 'd4e_invoice_print.account_invoices_with_qr']:
            if not data:
                data = {}
            data.setdefault('report_type', 'html')
            data = self._get_rendering_context(res_ids, data)
            if report_name == 'd4e_invoice_print.account_invoices_with_isr':
                doc_ids = data.get('doc_ids', [])
                if doc_ids:
                    pdf_content = self.env['account.move'].sudo()._pdf_invoices_with_isr(doc_ids)
                    return pdf_content, 'pdf'
                raise ValidationError('Select a document')
            elif report_name == 'd4e_invoice_print.account_invoices_with_qr':
                doc_ids = data.get('doc_ids', [])
                if doc_ids:
